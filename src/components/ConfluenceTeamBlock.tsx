...
interface ConfluenceTeamBlockProps {
  variant?: 'full' | 'cards-only'; // V5.20 : Permet d'afficher uniquement les cartes
}

export function ConfluenceTeamBlock({ variant = 'full' }: ConfluenceTeamBlockProps = {}) {
  const teamMembers: TeamMember[] = [
    {
      ...
      icon: <Award className="w-6 h-6 text-[#D1A65E]" strokeWidth={1.5} />,
    ...
    },
    {
      ...
      icon: <TrendingUp className="w-6 h-6 text-[#10b981]" strokeWidth={1.5} />,
    ...
    },
    {
      ...
      icon: <ShieldCheck className="w-6 h-6 text-[#A32E3A]" strokeWidth={1.5} />,
  ...
  return (
    <div className="w-full">
      {/* V5.20 : Titre et sous-titre conditionnels selon le variant */}
      {variant === 'full' && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {teamMembers.map((member, index) => (
          <div
            key={index}
            className="bg-white border border-[#E5E7EB] rounded-xl md:rounded-2xl p-6 md:p-8 transition-all duration-300 hover:shadow-lg hover:border-[#D1A65E]/30"
          ...
          >
            ...
            <div className="relative w-24 h-24 md:w-28 md:h-28 mx-auto mb-4 md:mb-6">
              <ImageWithFallback
                ...
                className="w-full h-full object-cover rounded-full border-4 border-[#D1A65E]/20"
              />
              {/* Badge Icône en bas à droite */}
              <div className="absolute -bottom-1 -right-1 w-10 h-10 md:w-12 md:h-12 bg-white border-2 border-[#D1A65E]/30 rounded-full flex items-center justify-center">
                {member.icon}
              </div>
            </div>
